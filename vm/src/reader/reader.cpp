...
namespace kette {
  ...
  auto Reader::read_word_raw() -> std::optional<RawWord> {
    ...
    for (auto const &c: sub) {
      if (!is_string && (c == ' ' || c == '\t' || c == '\n')) break;
      if (c == '\"') {
        if (is_string) { increment_column(); break; } else { is_string = true; }
      } 
      increment_column();
    }
    auto length = cursor - start_cursor;
    auto raw = source.substr(start_cursor, length);
    auto rawWord = RawWord {raw, start_line, start_column};
    return rawWord;
  }
  ...
  auto Reader::read_word() -> Word {
    ...
    // string
    if (raw.value.starts_with("\"")) return StringWord{std::string{raw.value}};
  ...
  auto to_string(const Word& word) -> std::string {
    ...
    else if (auto val = std::get_if<StringWord>(&word)) return val->value;
    else if (auto val = std::get_if<NumberWord>(&word)) 
      return std::visit([](auto&& num) -> auto { return std::to_string(num); }, *val);
    else return "None";
  }
} // namespace Kette
